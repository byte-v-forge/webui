...
- 本仓只提供 dashboard shell、布局、主题、导航、service catalog Web/API 基础入口和模块装载；基础组件、uikit、通用数据驱动组件和通用前端工具来自 `common-lib/ui`。
- 本仓不依赖基础设施仓或业务仓，不 import sibling repo 业务源码；允许通过 npm 包边界消费 `common-lib/ui`；不沉淀业务页面、基础设施页面、业务数据请求、业务动作、资源详情抽屉或 provider 分支。
...
- 能力差异由调用方传入声明式 capabilities/actions/required fields/required statuses 等元数据；本仓和 `common-lib/ui` 不得硬编码 Outlook、Cloudflare、GPT、GoPay 等业务/provider 判断。
